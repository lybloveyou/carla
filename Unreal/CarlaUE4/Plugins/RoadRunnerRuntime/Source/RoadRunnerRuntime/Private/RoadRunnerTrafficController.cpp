...
void URoadRunnerTrafficController::BeginPlay()
{
	...
	{
		Signals.Empty();
		TArray<URoadRunnerTrafficSignal*> signalComponents;
		GetOwner()->GetComponents<URoadRunnerTrafficSignal>(signalComponents);
		for (const auto& comp : signalComponents)
		{
			Signals.Emplace(comp->Id, comp);
		}
	}
	if (SignalToJunctionMap.Num() == 0)
	{
		TArray<URoadRunnerTrafficJunction*> junctionComponents;
		GetOwner()->GetComponents<URoadRunnerTrafficJunction>(junctionComponents);
		for (const auto& comp : junctionComponents)
		{
			Junctions.Emplace(comp->Id, comp);
		}
