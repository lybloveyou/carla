...
FLightBulbState FRoadRunnerTrafficUtil::LoadLightBulbState(FXmlNode* lightStateNode)
{
	...
	for (const auto& lightStateProperty : lightStateNode->GetChildrenNodes())
	{
		...
		else if (tag.Equals(TEXT("OnMesh"), ESearchCase::CaseSensitive)) {
			lightBulbState.OnMesh = lightStateProperty->GetContent();
		}
		else if (tag.Equals(TEXT("OffMesh"), ESearchCase::CaseSensitive)) {
			lightBulbState.OffMesh = lightStateProperty->GetContent();
		}
		else if (tag.Equals(TEXT("State"), ESearchCase::CaseSensitive))
		{
			FString stateString = lightStateProperty->GetContent();
			if (stateString.Equals(TEXT("On"), ESearchCase::CaseSensitive) || stateString.Equals(TEXT("true"), ESearchCase::CaseSensitive)) {
				lightBulbState.State = EnumBulbState::eOn;
			} 
			else if (stateString.Equals(TEXT("Blinking"), ESearchCase::CaseSensitive)) {
				lightBulbState.State = EnumBulbState::eBlinking;
			}
			else {
				lightBulbState.State = EnumBulbState::eOff;
			}
...
FSignalState FRoadRunnerTrafficUtil::LoadSignalState(FXmlNode* signalStateNode, const TMap<FString, USCS_Node*>& uuidToComponentMap, const TMap<FString, FSignalAsset>& uuidToSignalAssetMap)
{
	...
	for (const auto& lightBulbState : signalConfiguration.LightBulbStates)
	{
		...
		lightInstanceState.OnComponentName = FRoadRunnerTrafficUtil::FindByNamePrefix(uuidToComponentMap[signalState.Id], lightBulbState.Name);
		lightInstanceState.OffComponentName = FRoadRunnerTrafficUtil::FindByNamePrefix(uuidToComponentMap[signalState.Id], lightBulbState.Name);
