...
UENUM(BlueprintType)
enum class EnumBulbState : uint8 {
	eOn UMETA(DisplayName = "On"), 
	eOff UMETA(DisplayName = "Off"), 
	eBlinking UMETA(DisplayName = "Blinking")
};

// Various structs to organize data and make it editable in Unreal
////////////////////////////////////////////////////////////////////////////////
...
struct FLightBulbState
{
	...
	UPROPERTY(EditAnywhere, Category = "Traffic")
	EnumBulbState State = EnumBulbState::eOff;
};
...
////////////////////////////////////////////////////////////////////////////////
// Represents the "Signal" element under "SignalConfigurations" in the metadata
USTRUCT()
struct FSignalAsset
{
	GENERATED_USTRUCT_BODY()

	// UUID of the Signal asset
	UPROPERTY(EditAnywhere, Category = "Traffic")
	FString Id;
...
struct FLightInstanceState
{
	...
	// Default ctor to avoid crashes in 4.17
	FLightInstanceState()
	{
		OnRef.OtherActor = nullptr;
		OffRef.OtherActor = nullptr;
		LegacyRef.OtherActor = nullptr;
	};

	// Set after first lookup during runtime
	UPROPERTY(Transient)
	UMeshComponent* OnMeshComponent = nullptr;

	UPROPERTY(EditAnywhere, Category = "Traffic")
	FComponentReference OnRef;
	
	UPROPERTY(Transient)
	UMeshComponent* OffMeshComponent = nullptr;

	UPROPERTY(EditAnywhere, Category = "Traffic")
	FComponentReference OffRef;
	
	UPROPERTY(Transient)
	UMeshComponent* LegacyMeshComponent = nullptr;

	UPROPERTY(EditAnywhere, Category = "Traffic")
	FComponentReference LegacyRef;

	// Modified lightbulb name, set during import
	UPROPERTY(EditAnywhere, Category = "Traffic")
	FString OnComponentName;

	UPROPERTY(EditAnywhere, Category = "Traffic")
	FString OffComponentName;

	UPROPERTY(EditAnywhere, Category = "Traffic")
	FString LegacyComponentName;
...
class ROADRUNNERRUNTIME_API URoadRunnerTrafficJunction : public USceneComponent
{
	GENERATED_BODY()
...
public:
	...
	UPROPERTY(EditAnywhere, Category = "Traffic")
	URoadRunnerTrafficController* Controller;

	UPROPERTY(EditAnywhere, Category = "Traffic")
	TArray<FString> SignalIds;
...
public:
	...
	void SetController(URoadRunnerTrafficController* controller);
