...
namespace
{
	// Return null if we don't want to change the material
	UMaterialInterface* GetBaseMaterial(const FRoadRunnerImportUtil::MaterialInfo& materialInfo)
	{
		static FString carlaMaterialPath = "/Game/Carla/Static/GenericMaterials/Masters/";
		static FString roadrunnerMaterialPath = "/RoadRunnerCarlaContent/";
		...
		{
			materialName = "CarlaBaseMaterial";
			materialPath = roadrunnerMaterialPath;
		}
		...
		{
			// Foliage
			if (materialInfo.Name.Contains("Leaves")) // Hack to find leaves
			{
				materialName = "M_Foliage";
				materialPath = carlaMaterialPath;
			}
			else
			{
				return nullptr; // Just use RoadRunner materials
			}
		}
	...
	void CreateCarlaMaterial(FString sourceFilePath, FString packagePath, FRoadRunnerImportUtil::MaterialInfo materialInfo, int metadataVersion)
	{
		...
		// Check material to instance from
		UMaterialInterface* baseMaterial = GetBaseMaterial(materialInfo);
		if (!baseMaterial)
			return;
		...
		FString materialName = baseMaterial->GetName();
		if (materialName.Equals("CarlaBaseMaterial"))
		{
			if (materialInfo.TwoSided.Equals("true"))
			{
				unrealMaterial->BasePropertyOverrides.bOverride_TwoSided = true;
				unrealMaterial->BasePropertyOverrides.TwoSided = true;
			}
			...
			if (isTransparent)
			{
				unrealMaterial->BasePropertyOverrides.bOverride_BlendMode = true;
				if (materialInfo.ShadowCaster.Equals("false"))
				{
					unrealMaterial->BasePropertyOverrides.BlendMode = EBlendMode::BLEND_Translucent;
				}
				else
				{
					unrealMaterial->BasePropertyOverrides.BlendMode = EBlendMode::BLEND_Masked;
				}
			}
