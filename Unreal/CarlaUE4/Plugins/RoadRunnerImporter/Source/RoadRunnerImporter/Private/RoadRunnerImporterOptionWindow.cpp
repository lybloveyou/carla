...
using StyleSheet = FAppStyle;
#else
using StyleSheet = FEditorStyle;
#endif
...
void SRoadRunnerImporterOptionWindow::Construct(const FArguments & inArgs)
{
	...
	[
		...
		[
			...
			[
				SNew(SBorder)
				.BorderImage(StyleSheet::GetBrush("ToolPanel.GroupBorder"))
				.HAlign(HAlign_Left)
				.VAlign(VAlign_Top)
				[
					SNew(SImage)
			...
			[
				...
				[
					...
					[
						SNew(SBorder)
						.BorderImage(StyleSheet::GetBrush("ToolPanel.DarkGroupBorder"))
						.Padding(3)
						[
							SNew(STextBlock)
							.Text(LOCTEXT("OptionsTitle", "Import Options"))
							.TextStyle(&StyleSheet::GetWidgetStyle<FTextBlockStyle>("RichTextBlock.Bold"))
						]
					]
					+SVerticalBox::Slot()
					.HAlign(HAlign_Left)
					...
					[
						SNew(STextBlock)
						.Text(LOCTEXT("Disclaimer", "*Only compatible when importing as blueprint or level actors"))
					]
				]
			]
		]
