...
#if RR_VERSION_STARTING_AT(4,22)
#include <Subsystems/ImportSubsystem.h>
#endif

#if RR_VERSION_STARTING_AT(4,25)
#include "Layers/LayersSubsystem.h"
#else
#include <Editor/UnrealEd/Public/Layers/ILayers.h>
#endif

#if RR_VERSION_STARTING_AT(5,1)
...
namespace
{
	...
	void ImportOpenDriveMapping(URoadRunnerTrafficController* trafficController, const FString& openDriveFile)
	{
		...
		// Strip out <geoReference> so CDATA doesn't cause load failure
		FString openDriveBuffer;
		FFileHelper::LoadFileToString(openDriveBuffer, *openDriveFile);

		int startIndex = openDriveBuffer.Find(TEXT("<![CDATA["), ESearchCase::CaseSensitive, ESearchDir::FromStart);
		
		while (startIndex != -1)
		{
			int endIndex = openDriveBuffer.Find(TEXT("]]>"), ESearchCase::CaseSensitive, ESearchDir::FromStart);
			if (endIndex != -1)
			{
				openDriveBuffer.RemoveAt(startIndex, endIndex - startIndex + FString("]]>").Len());
			}
			else
			{
				UE_LOG(RoadRunnerImporter, Error, TEXT("Could not find closing tag for CDATA"));
			}
			startIndex = openDriveBuffer.Find(TEXT("<![CDATA["), ESearchCase::CaseSensitive, ESearchDir::FromStart);
		}

		FXmlFile* odXml = new FXmlFile(openDriveBuffer, EConstructMethod::ConstructFromBuffer);
		if (!odXml->IsValid())
		{
		...
		if (xmlSignalConfigs && xmlJunctionData)
		{
			...
			if (blueprint)
			{
				...
				for (const auto& kvp : signalToSignalAssetMap)
				{
					...
					component->IsLegacy = (metadataVersion < 5);
					component->BlueprintInit(kvp.Key, trafficController, kvp.Value, uuidToComponentMap);
			...
			else if (childActors.Num() != 0)
			{
				...
				while (traversalStack.Num() != 0)
				{
					...
					if (matcher.FindNext())
					{
						...
						signalComp->IsLegacy = (metadataVersion < 5);
