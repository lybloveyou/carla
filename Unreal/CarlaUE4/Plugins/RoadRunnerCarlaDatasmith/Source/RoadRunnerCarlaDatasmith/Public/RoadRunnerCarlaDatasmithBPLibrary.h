...
class URoadRunnerCarlaDatasmithBPLibrary : public UBlueprintFunctionLibrary
{
	...
	static FString GetSegmentationFolderName(const UStaticMesh& mesh, FString& segmentationType);

	// Categorize vehicle props to cityscape segmentation types.
	UFUNCTION(BlueprintCallable, meta = (DisplayName = "Categorize Vehicles", Keywords = "Datasmith CARLA Init"), Category = "RoadRunnerCarla")
	static void CategorizeVehicles(AActor* actor, TArray<UStaticMesh*>& outMeshes, FString& outFolderName);
	...
	// Categorize vehicles if CARLA version is greater than 0.9.13
	UFUNCTION(BlueprintPure, meta = (DisplayName = "Should Categorize vehicles", Keywords = "Datasmith CARLA Init"), Category = "RoadRunnerCarla")
	static bool ShouldCategorizeVehiclesToCityscapeOntology();
